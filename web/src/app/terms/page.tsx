...
export const metadata: Metadata = {
  ...
  description:
    "Hireschema Terms of Service — India-only platform. Governing law: India. Last updated January 2026.",
};
...
These Terms constitute a binding agreement between you and Hireschema, a company incorporated under the Companies Act 2013, India.`,
  },
  {
    id: "eligibility",
    title: "2. Eligibility",
    content: `Hireschema is available exclusively to:
• Indian residents with a valid +91 mobile number
• Persons aged 18 or older
• Individuals or companies legally permitted to use recruiting services under Indian law

By verifying your +91 number, you confirm you meet these requirements.`,
  },
  {
    id: "accounts",
...
• Comply with all applicable Indian employment laws, including equal opportunity requirements
• Not use Hireschema for bulk data harvesting or competitor intelligence

Violation of these rules may result in account termination and legal action under IT Act 2000 and DPDP Act 2023.`,
  },
  {
    id: "gmail-oauth",
    title: "6. Gmail OAuth usage",
    content: `When you connect your Gmail account, you grant Hireschema the gmail.send scope only. This allows Hireschema AI/Hireschema to send intro emails from your Gmail address.

We commit to:
• Never reading your inbox or any received messages
• Never sending emails without your explicit approval
• Revoking access immediately if you disconnect Gmail in settings
• Not storing email content beyond 30 days

You can revoke Gmail access at any time from Google Account settings > Third-party apps.`,
  },
  {
...
export default function TermsPage() {
  return (
    <main className="py-16 bg-paper-1">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-ink-900 mb-2">Terms of Service</h1>
          <p className="text-ink-500 text-sm">Effective: January 2026 · Governing law: India</p>
        </div>

        {/* TOC */}
        <nav className="bg-ink-50 rounded-2xl p-5 mb-10">
          <p className="text-xs font-semibold text-ink-500 uppercase tracking-wide mb-3">Contents</p>
          <ol className="space-y-1.5">
            {SECTIONS.map((s) => (
              <li key={s.id}>
                <a href={`#${s.id}`} className="text-sm text-accent hover:text-accent transition-colors">
                  {s.title}
                </a>
              </li>
            ))}
        ...
        <div className="mt-12 bg-ink-50 rounded-2xl p-6 border border-ink-100">
          <p className="text-ink-700 text-sm">
            Questions?{" "}
            <a href="mailto:hello@hireschema.com" className="text-accent underline">hello@hireschema.com</a>
            {" "}· Privacy:{" "}
            <a href="mailto:privacy@hireschema.com" className="text-accent underline">privacy@hireschema.com</a>
          </p>
        </div>
      </div>
    </main>
