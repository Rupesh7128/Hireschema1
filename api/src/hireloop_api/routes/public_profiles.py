...
from hireloop_api.services.invite_access import request_invite
from hireloop_api.services.public_cv_review import review_cv_bytes
...
@router.post("/invite-request", status_code=201)
async def public_invite_request(
    body: InviteRequestBody,
    request: Request,
    settings: Settings = Depends(get_settings),
    db=Depends(get_db),
) -> dict:
    """Collect an email for invite-only access (rate-limited)."""
    await _limit_public_request(request, db, settings, "public_invite_request", 10)
    try:
...
async def public_review_cv(
    request: Request,
    resume: UploadFile = File(...),
    settings: Settings = Depends(get_settings),
    db=Depends(get_db),
) -> dict:
    """Anonymous elite-recruiter CV review for /reviewmycv (gatekept, not stored)."""
    await _limit_public_request(request, db, settings, "public_review_cv", 6)
    ...
    try:
        ...
        logger.warning("public_review_cv_failed", error=str(exc)[:240])
        raise HTTPException(
            status_code=502,
            detail="Could not review that CV right now. Try a PDF or DOCX, or try again.",
        ) from exc
