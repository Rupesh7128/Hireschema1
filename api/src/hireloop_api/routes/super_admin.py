...
# ── Invite requests (invite-only waitlist) ───────────────────────────────────
...
async def update_invite(
    ...
    from hireloop_api.services.invite_access import set_invite_status

    try:
        updated = await set_invite_status(
            db,
            invite_id=invite_id,
            status=body.status,
...
async def create_invite(
    ...
    """Admin adds an email to the waitlist (optionally pre-approved)."""
    from hireloop_api.services.invite_access import request_invite, set_invite_status

    try:
        created = await request_invite(
            db,
            email=body.email,
    ...
    if body.approve and created.get("status") != "approved":
        updated = await set_invite_status(
            db,
