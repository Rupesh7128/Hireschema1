...
async def list_saved_jobs(
    ...
    for row in rows:
        row_dict = dict(row)
        raw = row_dict.pop("raw_data", None)
        origin = None
        ...
        if row_dict.get("overall_score") is None:
            ...
            if computed is not None:
                ...
                if origin == "chrome_extension":
                    computed["saved_via_extension"] = True
                    if not computed.get("action_label"):
                        computed["action_label"] = "Saved from Chrome"
                cards.append(computed)
                continue
            row_dict["overall_score"] = 0.0
        card = serialize_job_card(row_dict)
        if origin == "chrome_extension":
            card["saved_via_extension"] = True
            if not card.get("action_label"):
                card["action_label"] = "Saved from Chrome"
        cards.append(card)
    return cards
