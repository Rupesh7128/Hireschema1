...
async def fetch_instant_shelf(
    ...
    *,
    ...
    shelf = cards[:limit]
    # Persist so Matches API / refresh still show first-session jobs (starter
    # pad previously skipped match_scores — only job_search path wrote them).
    if shelf:
        try:
            await aarya_tools._persist_chat_match_scores(
                db,
                candidate_id=candidate["id"],
                rows=shelf,
            )
