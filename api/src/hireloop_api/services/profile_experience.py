...
Each role gets an ``aarya_insights`` bullet list — Hireschema AI's read on what that stint
...
def _aarya_role_insights(role: dict[str, Any], skills: list[str]) -> list[str]:
    """Deterministic Hireschema AI bullets when CI/LLM insights aren't stored yet."""
    ...
    bullets.append(
        ...
        "Hireschema AI weights it heavily for role matching."
    )
    ...
    if len(bullets) < 3:
        bullets.append(
            "Upload more detail or chat with Hireschema AI to sharpen the story for this chapter."
        )
