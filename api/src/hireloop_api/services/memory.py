...
Hireschema AI should remember what a candidate tells her — not just inside one chat, but
...
job platform, based on what they tell Hireschema AI (the AI career assistant) in chat.
...
self-learning loop: every chat turn refines what Hireschema AI remembers user-wide.
"""
...
async def run_memory_update(settings: Settings, candidate_id: str, conversation_id: str) -> None:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # If the candidate told us something new (e.g. answered one of Hireschema AI's
...
# Friendly labels + render order for career_facts when surfaced to Hireschema AI.
...
def format_known_facts(
    ...
    *,
    ...
    Hireschema AI uses what we already know (preferred name, work mode, target role,
...
def _build_brief(
    ...
    convo = "\n".join(
        f"{'Candidate' if m['role'] == 'user' else 'Hireschema AI'}: {m['content']}" for m in messages
    )
    return (
        "CURRENT PROFILE:\n"
        + "\n".join(prof_lines)
