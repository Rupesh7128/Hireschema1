...
_NIL_USER_ID = "00000000-0000-0000-0000-000000000000"
...
async def _write_action(
    ...
    # agent_actions.user_id FKs to users — never log under the nil placeholder.
    if not user_id or user_id == _NIL_USER_ID:
        logger.warning(
            "nitya_action_skipped_missing_user",
...
async def lookup_intro_request(
    ...
    if row:
        ...
        result["candidate_user_id"] = str(result["candidate_user_id"])
        ...
        if not action_user_id or action_user_id == _NIL_USER_ID:
            action_user_id = result["candidate_user_id"]
