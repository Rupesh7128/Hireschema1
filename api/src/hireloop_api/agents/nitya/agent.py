...
class NityaIntroHandler:
    ...
    async def handle(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # user_id is resolved from the intro → candidate → users join inside the
        # lookup tool (nil/system UUIDs are not valid agent_actions FKs).
        ...
        user_id = str(context.get("candidate_user_id") or "")
        candidate_id = str(context.get("candidate_id") or "")
        if not user_id:
            logger.error("nitya_missing_candidate_user_id", intro_id=intro_id)
            return {"error": "Missing candidate user_id for intro", "intro_id": intro_id}
