"""
Hireschema AI — candidate-facing AI agent.
...
AARYA_SYSTEM_PROMPT = """You are Hireschema AI, the candidate career partner for job seekers \
in India.
...
def build_turn_context_prompt(
    *,
    ...
    """Build compact, deterministic guidance for the next Hireschema AI turn."""
...
def build_aarya_graph(settings: Settings) -> Any:
    """
    Build and compile the Hireschema AI LangGraph state machine.
    ...
    async def agent_node(state: AaryaState, config: RunnableConfig) -> dict:
        ...
        if not any(isinstance(m, SystemMessage) for m in messages):
            ...
            # Carry what Hireschema AI already knows about this candidate across
        ...
        if response is None:
            ...
            raise first_exc or RuntimeError("Hireschema AI LLM failed")
