...
import {
  isInviteAccessError,
  redirectForInviteAccess,
} from "@/lib/auth/redirect-invite-access";
...
/** Product signup is candidate-only. */
...
export function SignupForm() {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          // round-trip keeps candidate intent.
  ...
  async function handleVerifyCode(e: React.FormEvent) {
    ...
    try {
      ...
      if (isInviteAccessError(error)) {
        clearSignupRole();
        clearPostAuthRedirect();
        await redirectForInviteAccess(supabase, (href) => router.replace(href), error);
        return;
      }
  ...
  async function handleDevEmailSignIn(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        ...
        if (isInviteAccessError(bootErr)) {
          clearSignupRole();
          clearPostAuthRedirect();
          await redirectForInviteAccess(supabase, (href) => router.replace(href), bootErr);
          return;
        }
        setErrorMessage(
          bootErr instanceof Error ? bootErr.message : "Signed in but account setup failed.",
        );
        return;
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {!isSignIn ? (
        <div className="space-y-4">
          <p className="text-small leading-relaxed text-ink-600">
            Hireschema is <span className="text-ink-900">invite-only</span>. Request access with
            your email — once approved, come back here to sign in.
          </p>
          <Link
            href="/invite"
            className={cn(BTN_PRIMARY, "inline-flex w-full justify-center px-5 py-3 text-body")}
          >
            Request an invite
          </Link>
      ...
      <p className="text-small text-ink-600">
          Welcome back — sign in with the email that was approved for your invite.
        </p>
      ...
      <p className="text-xs text-ink-500 text-center">
        {isSignIn ? (
          <>
            Need access?{" "}
            <Link href="/invite" className="font-medium text-accent hover:underline">
              Request an invite
            </Link>
          </>
        ) : null}
