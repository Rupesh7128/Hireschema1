...
import { KitDoneMoment } from "./KitDoneMoment";
...
export function MatchFeed({
  ...
  const [kitDoneJob, setKitDoneJob] = useState<MatchedJob | null>(null);
  ...
  const {
    ...
    onKitReady: (job) => setKitDoneJob(job),
  });
  ...
  useEffect(() => {
    if (loading || error || jobs.length > 0 || seedJobs?.length || emptyRefreshCount >= 5) return;
    const id = window.setTimeout(() => {
      setEmptyRefreshCount((count) => count + 1);
      void load(true);
    }, 12_000);
    return () => window.clearTimeout(id);
  }, [emptyRefreshCount, error, jobs.length, load, loading, seedJobs?.length]);

  // First-session guarantee: if the feed is still empty after the first load
  // and we have no seed, nudge Hireschema AI to search once.
  useEffect(() => {
    if (compact || loading || error || jobs.length > 0 || seedJobs?.length) return;
    if (!onAskAarya || emptyRefreshCount !== 1) return;
    onAskAarya();
  }, [
    compact,
    emptyRefreshCount,
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      ...
      {seedJobs?.length ? (
        <p className="text-micro text-ink-500 mb-3 shrink-0">
          {seedTitle
            ? `Based on your CV — ${seedJobs.length} role${seedJobs.length === 1 ? "" : "s"} to review (incl. ${seedTitle}).`
            : `Based on your CV — ${seedJobs.length} role${seedJobs.length === 1 ? "" : "s"} to review.`}
        </p>
      ) : null}
          ...
          <div className="mt-2 space-y-3">
            ...
            {historyLoading ? (
              ...
              <p className="text-micro text-ink-500 pb-2">
                Past matches will show here once Hireschema AI has scored roles for you.
              </p>
            )}
          </div>
        ) : null}
      ...
      <KitDoneMoment
        open={Boolean(kitDoneJob)}
        onClose={() => setKitDoneJob(null)}
        jobTitle={kitDoneJob?.title ?? "this role"}
        companyName={kitDoneJob?.company_name}
        onPreview={() => {
          if (kitDoneJob) void openKitPreview(kitDoneJob, "resume");
        }}
      />
    </div>
  );
