...
interface JobCardProps {
  ...
  applyLocked?: boolean;
  onApplyLocked?: () => void;
  /** chat = compact actions for Hireschema AI thread; feed = full matches panel */
  variant?: "feed" | "chat";
...
export function JobCard({
  ...
  const statusLabel =
    historyDateLabel ??
    (job.saved_via_extension ? "Saved from Chrome" : null) ??
    job.action_label ??
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-2 pt-3 border-t border-ink-100">
        ...
        {!isChat && onTailorResume && (
          ...
          >
            {tailoredReady ? "Preview kit" : tailoring ? "Preparing…" : "Application kit"}
          </Button>
        )}
