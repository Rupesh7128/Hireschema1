 ...
 *
 *   1. Analysis — show what Hireschema AI extracted from the CV
 ...
 *
 * On completion the parent (ChatInterface) starts the normal Hireschema AI chat search,
...
export function CareerKickoffFlow({
  ...
  const reviewRows = useMemo(
    () =>
      [
        ...
        { label: "Hireschema AI prompt", value: selected[0] ? buildRoleSearchPrompt(profile, selected[0]) : null },
      ].filter((r) => r.value),
    [c, selected, profile],
  );
  ...
  // ── Step 3 confirm: ask Hireschema AI through the normal chat search path ───────────
  ...
  return (
    <FlowShell stepIndex={stepIndex}>
      ...
      {step === "review" && (
        <>
          <p className="text-small text-ink-800 leading-relaxed">
            Last check — this is the search brief I&apos;ll send to Hireschema AI now.
    ...
    <div className="w-full animate-fade-in">
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="text-small font-semibold text-ink-900">
          Hireschema AI <span className="font-normal text-ink-500">· getting started</span>
        </p>
        {stepIndex != null && (
