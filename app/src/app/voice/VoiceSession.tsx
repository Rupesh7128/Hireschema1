...
/**
 * VoiceSession — 15-min Hireschema AI voice career session.
 *
 ...
 *   Same Hireschema AI agent, memory, and tools as ChatInterface.
 *
...
type TurnState =
  ...
  | "processing"     // streaming Hireschema AI's reply
...
export function VoiceSession({
  ...
  const [aaryaText, setAaryaText]     = useState<string>("");  // last Hireschema AI reply
  ...
  /**
   * Stream a message through Hireschema AI, return the full reply text.
  ...
  /** One full turn: stream Hireschema AI's reply → speak it → hand the mic back. */
  const doTurn = useCallback(
    async (conversationId: string, triggerText: string): Promise<void> => {
      if (isEndingRef.current) return;

      // ── Hireschema AI thinks (and starts speaking mid-stream) ─────────────────────
      setTurnState("processing");
      setStreamStatus("Hireschema AI is thinking…");
      setAaryaText("");
      setTranscript("");

      // Sentence-streaming TTS: each complete sentence is appended to a serial
      // speech chain the moment it arrives, so Hireschema AI starts talking ~1-2s into
      ...
      try {
        ...
        setErrorMsg("Couldn't reach Hireschema AI. Tap the mic to retry.");
  ...
  const persistCompletion = useCallback(async () => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        setErrorMsg(
          `${(error as Error).message || "Hireschema AI couldn't save the call."} Your call is still active on the server. Retry saving to finish.`
        );
  ...
  const startCall = useCallback(async () => {
    ...
    try {
      ...
      const greeting = resumed
        ...
        : `Hi ${firstName}, this is Hireschema AI — I'm a senior recruiter here at Hireschema. ` +
      ...
      if (isMountedRef.current) {
        clearCallTimer();
        setErrorMsg((error as Error).message || "Couldn't connect to Hireschema AI. Please retry.");
        setTurnState("idle");
      }
    } finally {
  ...
  /** Tap mic: barge-in while Hireschema AI speaks, or force-stop capture while listening */
  ...
  const statusLabel: Record<TurnState, string> = {
    idle:           "Ready to start",
    starting:       "Connecting to Hireschema AI…",
    aarya_speaking: "Hireschema AI is speaking…",
    user_listening: "Your turn — speak now",
    processing:     "Hireschema AI is thinking…",
  ...
  return (
    ...
    >
      <div className="max-w-sm w-full flex flex-col items-center gap-8">
        ...
        {/* ── Hireschema AI avatar + animated rings ────────────────────────────── */}
        <div className="relative flex items-center justify-center">
          {/* Outer pulse when Hireschema AI is speaking */}
        ...
        <div className="text-center space-y-1">
          <h1 className="text-h2 font-semibold text-ink-900">
            {isActive || isConnecting ? "Hireschema AI" : "Talk to Hireschema AI"}
          </h1>
          {isActive && (
        ...
        {/* While the user speaks, bar heights track the real mic level
            (audioLevel 0..1). While Hireschema AI speaks, they animate on a loop. */}
        {isActive && (
        ...
        {/* Real-time preview: Hireschema AI's reply streams in as she thinks, and the
            ...
            <div className="w-full space-y-2" aria-live="polite">
              {aaryaText && (
                <div className="bg-paper-1 border border-ink-100 rounded-xl px-4 py-3">
                  <p className="text-micro text-ink-400 uppercase tracking-wide mb-1">Hireschema AI</p>
        ...
        {!canListen && turnState === "idle" && (
          <div className="w-full bg-ink-50 border border-ink-200 rounded-xl px-4 py-3 text-small text-ink-700 text-center">
            Voice input requires Chrome or Edge. You can still listen to Hireschema AI and type your responses in the dashboard chat.
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-5">
          ...
          {turnState === "idle" && (
            <button
              ...
              aria-label="Start call with Hireschema AI"
            >
        ...
        {turnState === "idle" && (
          <div className="text-center space-y-1.5">
            ...
            <p className="text-small text-ink-500 leading-relaxed max-w-xs">
              Hireschema AI will guide you through a 15-minute conversation about your
