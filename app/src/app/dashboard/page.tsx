...
export default async function DashboardPage({
  ...
  if (!token) {
    redirect("/signup?mode=signin&redirect=/dashboard");
  }

  // Invite-only: Auth session can exist before approval. Try bootstrap; if the
  // API says invite is pending, show thank-you. Approved users get provisioned.
  if (!profile && token) {
    let bootRes: Response | null = null;
    try {
      bootRes = await fetch(`${API_URL}/api/v1/auth/bootstrap`, {
        method: "POST",
        headers: {
    ...
    if (bootRes?.status === 403) {
      const body = (await bootRes.json().catch(() => ({}))) as {
        detail?: { code?: string; status?: string } | string;
      };
      const detail = body.detail;
      const status =
        typeof detail === "object" && detail?.status === "rejected"
          ? "rejected"
          : "pending";
      redirect(`/invite?thanks=1&status=${status}`);
    }
