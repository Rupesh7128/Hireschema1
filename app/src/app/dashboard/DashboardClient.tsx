...
export function DashboardClient({
  ...
  // Jobs Hireschema AI surfaced in chat — mirrored into the Matches sidebar so the user
  ...
  useEffect(() => {
    ...
    // Surface Matches immediately so first session never looks empty.
    setActivePanel("jobs");
  }, []);
  ...
  function handleRequestIntro(job: MatchedJob) {
    ...
    void saveJob(job.job_id).catch(() => {
      /* Hireschema AI intro path also bookmarks server-side */
    });
    toast.success(
      "Intro started — emails send from your Gmail (higher reply rates). Draft opens next.",
    );
    ...
    void createCandidateIntro(job.job_id)
      .then((result) => {
        if (result.intro_id) {
          ...
          // Surface Intros so the draft is visible before Gmail connect.
          openPanel("inbox");
        }
      })
      .catch((e) => {
