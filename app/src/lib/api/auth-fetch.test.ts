...
describe("apiAuthFetch error classification", () => {
  ...
  it("chat error sanitization stays purpose-specific (not Railway/API-host copy)", () => {
    ...
    expect(
      sanitizeChatError("Hireschema AI is busy right now — wait a few seconds and try again."),
    ).toBe("Hireschema AI is busy right now — wait a few seconds and try again.");
    expect(sanitizeChatError("Hireschema AI took too long to respond. Please try again.")).toBe(
      "Hireschema AI took too long to respond. Please try again.",
    );
