/**
 * Unified Hireschema AI chat transport — single SSE client for text chat + voice turns.
 *
...
function parseApiErrorDetail(res: Response, body: unknown): string {
  ...
  if (res.status === 429) {
    return "Hireschema AI is busy right now — wait a few seconds and try again.";
  }
  if (res.status === 404) {
...
/** Return the user's canonical Supabase-backed Hireschema AI conversation id. */
...
/**
 * Stream one user turn to Hireschema AI. Returns the full assistant reply text.
      ...
      return await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(new Error("Hireschema AI took too long to respond. Please try again."));
