...
export async function updateSession(request: NextRequest) {
  ...
  // or invite thank-you / pending state after a blocked sign-in).
  const authPaths = ["/signup", "/login"];
  const isAuthPage = authPaths.some((p) => pathname.startsWith(p));
  const stayOnAuth =
    request.nextUrl.searchParams.has("error") ||
    request.nextUrl.searchParams.get("invite") === "pending" ||
    request.nextUrl.searchParams.get("thanks") === "1";

  if (isAuthPage && user && !stayOnAuth) {
    const url = request.nextUrl.clone();
