...
Public endpoint abuse models (keep in sync with R16):

| Endpoint | Auth | Rate limit | Abuse model |
|---|---|---|---|
| `POST /api/v1/public/review-cv` | none (IP hash) | **6/h** via `api_rate_limits` bucket `public_review_cv` | Anonymous CV upload + free/fast OpenRouter; file validated (`validate_resume_upload`, 10MB); **not stored**; gatekept response; LLM fallback to heuristic if providers fail |
| `POST /api/v1/public/invite-request` | none (IP hash) | **10/h** via `api_rate_limits` bucket `public_invite_request` | Invite-only waitlist; stores email (+ optional name/note); no auth account created; spam limited by rate + unique email |
